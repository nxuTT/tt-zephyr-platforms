...
/* program_noc_dma_tlb must be invoked before this func call */
static inline bool noc_wait_cmd_ready(void)
{
	uint32_t cmd_ctrl = 0;
	bool timeout = false;
	uint64_t deadline = k_uptime_get() + NOC_DMA_TIMEOUT_MS;
	while (cmd_ctrl && !timeout) {
...
{
	return y << NOC_ADDR_NODE_ID_BITS | x;
}

static uint32_t noc_dma_format_multicast(uint8_t start_x, uint8_t start_y, uint8_t end_x, uint8_t end_y)
{
	return end_x | (end_y << NOC_ADDR_NODE_ID_BITS) |
	       (start_x << 2* NOC_ADDR_NODE_ID_BITS) |
	       (start_y << 3*NOC_ADDR_NODE_ID_BITS);
}

static bool noc_dma_transfer(uint32_t cmd, uint32_t ret_coord, uint64_t ret_addr,
			     uint32_t targ_coord, uint64_t targ_addr, uint32_t size,
			     bool multicast, uint8_t transaction_id, bool include_self)
{
...
void noc_dma_read(uint8_t local_x, uint8_t local_y, uint64_t local_addr,
		  uint8_t remote_x, uint8_t remote_y, uint64_t remote_addr, uint32_t size)
{
	...
	noc_dma_transfer(NOC_CMD_WR, ret_coord, ret_addr, targ_coord, targ_addr, size,
			 false, 0, false);
}

void noc_dma_write(uint8_t local_x, uint8_t local_y, uint64_t local_addr,
		   uint8_t remote_x, uint8_t remote_y, uint64_t remote_addr, uint32_t size)
{
...
static void noc_dma_write_multicast(uint8_t local_x, uint8_t local_y, uint64_t local_addr,
			     uint8_t remote_start_x, uint8_t remote_start_y, uint8_t remote_end_x,
			     uint8_t remote_end_y, uint64_t remote_addr, uint32_t size, bool include_self)
{
	uint32_t ret_coord = noc_dma_format_multicast(remote_start_x, remote_start_y, remote_end_x, remote_end_y);
	uint64_t ret_addr = remote_addr;
...
void noc_dma_broadcast(uint8_t local_x, uint8_t local_y, uint64_t addr, uint32_t size)
{
	/* Skip ARC on column x = 8 */
	uint8_t remote_start_x = 9;
	uint8_t remote_end_x = 7;
	uint8_t remote_start_y = 0;
	uint8_t remote_end_y = NOC_Y_SIZE - 1;

	uint64_t local_addr = addr;
	uint64_t remote_addr = addr;
	noc_dma_write_multicast(local_x, local_y, local_addr, remote_start_x, remote_start_y,
